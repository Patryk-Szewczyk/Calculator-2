...
const Layout_DESKTOP: {
    ...
    setMenuButton_AEL(): void {
        const menuButtonBody_EL: HTMLDivElement = document.querySelector('div.dsk-menu-button-box');
        const menuButton_EL: HTMLDivElement = document.querySelector('svg.dsk-menu-arrow');
        const calcDott_EL: HTMLDivElement = document.querySelector('div.menu-button-dott');
        const appCover_EL: HTMLDivElement = document.querySelector('div.app-cover');
        let isShow: boolean = false;
        [calcDott_EL, menuButton_EL].forEach((element) => {
...
const buttonHover: {
    ...
    setButtonHover_AEL(): void {
        const calcButton_COLL: NodeListOf<HTMLDivElement> = document.querySelectorAll('div.buttons-group > .item > div');
        for (let i: number = 0; i < calcButton_COLL.length; i++) {
            calcButton_COLL[i].addEventListener("mouseover", () => {
                calcButton_COLL[i].style.cursor = "pointer";
            }, false);
            calcButton_COLL[i].addEventListener("mouseout", () => {
                calcButton_COLL[i].style.cursor = "default";
            }, false);
            ['click', 'touchend'].forEach((ev) => {
                calcButton_COLL[i].addEventListener(ev, () => {
...
const Calculator_MathLogic_FUNCTIONS: {
    ...
    operation_TABLE: Function,
    mode_EL: HTMLElement,
    calc_MODE: string,
    isResultTAU: boolean,
    screen_INFO_EVA: string,
    screen_INFO_TAU: string,
    //expressionForm_AR: string,  // bracketWord_AR
    expressionResult_AR: string[],
    //expressionResult_GROUP_AR: string[][],
    //statistics_TAU_AR: [{expression: string, result_AR: string[]}][],
    TAU_TYPE: number
} = {
    bt_EVT: undefined,
    bt_DIV: null,
                    ...
                    case "=": {
                            if (this.isValid == true) {
                                ...
                                if (this.calc_MODE == "TAU") {
                                    ...
                                    this.statistics_TAU_AR = [];
                                    //this.ex_idx = -1;
                                }
                                this.operation_CALCULATE_1(this.bt_ID);
                                (this.calc_MODE == "TAU") ? this.operation_TABLE("(" + this.value + ")") : null;
                            }
                        } break;
                    case "BACK":
    ...
    operation_MODE(): void {
        const hover_EL: HTMLDivElement = document.createElement('div');
    ...
    operation_TABLE(exp): void {
        ...
        // Grupowanie wyników:
        let joinExp: string = "";
        let splitPlace_TARGET: number = this.TAU_TYPE;  // 2 | 4 | 8
        let splitPlace_COUNTER: number = 0;
        for (let i: number = 0; i < this.expressionResult_AR.length; i++) {
